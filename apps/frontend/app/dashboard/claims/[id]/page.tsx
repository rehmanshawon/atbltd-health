    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="bg-white border border-gray-200 rounded-md p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span
            className={`inline-flex px-2.5 py-0.5 rounded-md text-xs font-medium border ${STATUS_COLORS[claim.status] || STATUS_COLORS.submitted}`}
          >
            {claim.status
              .replace(/_/g, " ")
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          {claim.operationDate && (
            <div>
              ...
              <span className="text-gray-700 ml-1">
                {formatDate(claim.operationDate)}
              </span>
            </div>
          )}
          {claim.doctorName && (
      ...
      <div className="bg-white border border-gray-200 rounded-md p-6">
        <h2 className="text-lg font-semibold text-brand-blue mb-1">
          Upload Documents
        </h2>
        ...
        <div className="space-y-3">
          {uploadEntries.map((entry) => (
            <div
              key={entry.id}
              className="flex items-start gap-3 p-4 bg-gray-50 rounded-md"
            >
              <div className="flex-1 space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateUploadEntry(entry.id, "documentType", e.target.value)
                  }
                ...
                <input
                  ...
                  value={entry.notes}
                  onChange={(e) =>
                    updateUploadEntry(entry.id, "notes", e.target.value)
                  }
